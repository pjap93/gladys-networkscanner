...
module.exports = function checkUsersPresence(){
    ...
    return gladys.param.getValue('USER_TIME_BEFORE_CONSIDERING_LEFT_HOME')
        ...
        .spread((timeBeforeLeftInMinute, houses) => {
            ...
            return [houses, Promise.map(houses, function(house) {
		return gladys.utils.sql(queries.getUserAtHomeAndNotSeenSince, [house.id, house.id, 'networkscanner', timeBeforeLeftInMinute]);
                })
            ];
        })
